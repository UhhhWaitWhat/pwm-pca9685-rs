	...
	I2C: hal::blocking::i2c::Write<Error = E> + hal::blocking::i2c::WriteRead<Error = E>,
{
	...
	pub fn set_channel_on_off(
		...
		let reg = get_register_on(channel);
		let (_, cur_off) = self.read_two_double_registers(reg)?;

		let full_off = cur_off & FULL_ON_OFF_BITMASK;

		self.write_two_double_registers(reg, on, off | full_off)
	}
	...
	pub fn set_channel_full_on(&mut self, channel: Channel, value: u16) -> Result<(), Error<E>> {
		...
		self.write_double_register(reg, value | FULL_ON_OFF_BITMASK)
	}
	...
	pub fn set_channel_full_off(&mut self, channel: Channel) -> Result<(), Error<E>> {
		let reg = get_register_off(channel);
		let val = self.read_double_register(reg)?;

		self.write_double_register(reg, val | FULL_ON_OFF_BITMASK)
	}

	/// Clear the channels always off flag.
	///
	/// See section 7.3.3 "LED output and PWM control" of the datasheet for
	/// further details.
	pub fn clear_channel_full_off(&mut self, channel: Channel) -> Result<(), Error<E>> {
		let reg = get_register_off(channel);
		let val = self.read_double_register(reg)?;
